ruleset a169x522 {
  meta {
    name "kQuery QRCode"
    description <<
      jQuery QRCode Generator Plugin for KRL
    >>

    provides init, qrcode
  ...
  global {
    ...
    qrcode = defaction(selector, text) {
      configure using render = "canvas"
                and   height = 256
                and   width  = 256
...
a.isDark(e,h)?"#000000":"#ffffff").appendTo(f)}a=c}$KOBJ(a).appendTo(this)})}})($KOBJ);
        |>;
      }
    };
